...
def extract_digit(cell, debug=True):
	...
	for i in range(4):
		detected=False
		for j in range(25):
			if thresh[i,j]!=0 and thresh[i,j+1]!=0 and thresh[i,j+2]!=0:
				detected=True
				break
		if detected:
			for k in range(28):
	...
	for i in range(4):
		detected=False
		for j in range(25):
			if thresh[j,i]!=0 and thresh[j+1,i]!=0 and thresh[j+2,i]!=0:
				detected=True
				break
		if detected:
			for k in range(28):
	...
	for i in range(6):
		for j in range(6):
			tmp=cv2.countNonZero(thresh[i:i+22,j:j+22])
			if tmp>=count:
				count=tmp
				max_i=i
				max_j=j
	thresh=cv2.resize(thresh[max_i:max_i+22,max_j:max_j+22],(28,28))
	#kernel=np.ones((2,2),np.uint8)
	#thresh=cv2.dilate(thresh,kernel=kernel)
	(h, w) = thresh.shape
	percentFilled = cv2.countNonZero(thresh) / float(w * h)
